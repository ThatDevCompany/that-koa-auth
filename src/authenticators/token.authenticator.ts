 ...
 */
export interface TokenAuthNService<U extends User, T extends Tenant> extends AuthService {
 ...
 */
export class TokenAuthenticator<U extends User, T extends Tenant> implements Authenticator<U, T> {
	...
	async generateAuthContext(credentials: Credentials<T>): Promise<AuthContext<U, T>> {
		...
		const user: U = await this.auth.findUserByToken(credentials.identity, credentials.tenant)
